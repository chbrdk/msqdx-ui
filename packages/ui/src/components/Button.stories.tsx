...
export const LargePill: Story = {
  args: { size: 'lg', shape: 'pill', children: 'Continue' },
}
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      {(['primary', 'ghost', 'subtle', 'danger', 'link'] as const).map((variant) => (
        <div key={variant} style={{ display: 'flex', flexWrap: 'wrap', gap: '0.65rem', alignItems: 'center' }}>
          <span style={{ width: '4.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--muted)' }}>
            {variant}
          </span>
