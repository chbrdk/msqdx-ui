...
- DS deposit as Brandion guideline / Plexon Collection (program **DS-DEPOSIT**, D0–D3 shipped 2026-08-13): canonical `brandion-v3/knowledge/ds-tokens-as-plexon-project.md` · path map `brandion-v3/knowledge/ds-deposit-path-map.md` · DTCG mirrors `packages/ui-tokens/deposit/msq-dx-deposit.dtcg.json` + `msq-dx-deposit-light.dtcg.json` (Brandion import SoT remains `brandion-v3/packs/…`) · D3a light twin · brand colors `knowledge/msqdx-brand-colors.md`
