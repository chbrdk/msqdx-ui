...
- Mag PDF kit (react-pdf): `@msqdx/ui/mag` · `packages/ui/src/mag/` · fonts `packages/ui/src/mag/fonts/` · shared colors `packages/ui/src/magazine/colors.ts` · twin map `packages/ui/src/magazine/twins.ts` · theme `MagThemeProvider` · docs Storybook `Mag/` · Drift-CI `mag/mag-kit.test.tsx` · spec `specs/domain/msqdx-ui-mag-pdf-kit.md` · twin knowledge `knowledge/print-magazine-twins.md` · **report primitives wave P92** (Phase A–E done) `specs/domain/msqdx-ui-print-report-primitives.md` · `knowledge/print-report-primitives-wave.md` · consumers plexon `eqc-magazine-pdf.tsx` · creation `lib/magazine-pdf/`
...
- Shared settings layout: `packages/ui/src/components/SettingsShell.tsx` + `themePreference.ts` + `accentPreference.ts` + `AccentSwatchGroup`
